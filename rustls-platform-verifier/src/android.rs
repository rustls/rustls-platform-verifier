...
impl From<JNIError> for Error {
    ...
    fn from(cause: JNIError) -> Self {
        if let JNIError::JavaException = cause {
            ...
            if let Ok(mut guard) = unsafe { global().vm().get_env_attachment(&mut scope) } {
...
impl<'a, 'env> LocalContext<'a, 'env> {
    ...
    fn load_class(&mut self, name: &'static CStr) -> Result<JClass<'env>, Error> {
        // CachedClass names are ASCII constants, always valid UTF-8.
        let name = self.env.new_string(name.to_str().unwrap())?;
        self.global
            .loader
